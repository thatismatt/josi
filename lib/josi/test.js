  ...
  run: function() {
    ...
    if (totalFails) {
      failDetails.forEach(function(fd) {
        ...
        var stack = fd.error.stack.split('\n');
        var maxStackHeight = 3;
        if (stack.length > maxStackHeight) {
          stack = stack.splice(0, maxStackHeight);
          stack.push('    ...');
        }
        stack = stack.join('\n');
        sys.puts(stack);
...
this.createMockFunction = function(name) {
  var callCount = 0;
  var mock = function() { callCount++; };
  mock.isCalled = function(n) { return callCount != (n || 0); };
  mock.getName = function() { return name; };
  return mock;
};
