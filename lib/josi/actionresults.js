...
this.ViewResult = ViewResult = ActionResult.extend({
  init: function(data, view, master) {
    this._super();
    // todo: work out a 'correct' response to no data
    // note: if you remove '|| {}' then in execute below the templating.simple
    //       function creates a templating function rather than a string
    this.data = data || {};
    this.view = view;
    this.master = master;
  },
  execute: function(req, res) {
    // fixme: remove hardcoded dependency on this particular templating engine
