    ...
    http
      .createServer(function(req, res) {
        try {
          ...
          var routeResult = app.router.route(urlSplit[0]);
          var actionContext = {
            route: routeResult.route,
            query: querystring.parse(urlSplit[1]),
            form: {} // todo: extract form variables
          }
          if (routeResult instanceof RouteResult) {
            if (routeResult instanceof MissingRouteResult) {
              var actionResult = actionresults.notFound(routeResult.message || 'No route matched the url: ' + urlSplit[0]);
            } else {
